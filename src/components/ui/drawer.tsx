...
type SwipeDirection = "dynamic" | DrawerPrimitive.Root.Props["swipeDirection"];

type DrawerContextProps = {
  hasSnapPoints: boolean;
  modal: DrawerPrimitive.Root.Props["modal"];
  showSwipeHandle: boolean;
  swipeDirection: NonNullable<SwipeDirection>;
};
...
function Drawer({
  ...
  const swipeDir: DrawerPrimitive.Root.Props["swipeDirection"] =
    swipeDirection === "dynamic" ? (md ? "right" : "down") : swipeDirection;

  const hasSnapPoints = snapPoints != null && snapPoints.length > 0;
  const contextValue = React.useMemo(
    () => ({ hasSnapPoints, modal, showSwipeHandle, swipeDirection: swipeDir }),
    [hasSnapPoints, modal, showSwipeHandle, swipeDir],
  );
...
function DrawerTitle({ className, ...props }: DrawerPrimitive.Title.Props) {
  return (
    <DrawerPrimitive.Title
      ...
      className={cn("font-heading text-base font-medium text-fg", className)}
      {...props}
    />
  );
}
...
function DrawerDescription({ className, ...props }: DrawerPrimitive.Description.Props) {
  return (
    <DrawerPrimitive.Description
      ...
      className={cn("text-sm text-balance text-muted-fg", className)}
      {...props}
    />
  );
}

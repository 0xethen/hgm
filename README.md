### www

The repository for the official HackGwinnett marketing website ([hackgwinnett.org](https://hackgwinnett.org))

Non-contributors, **please report issues at [hackgwinnett.org/go/issues](https://hackgwinnett.org/go/issues)**

> remaining TODOs: sitemap, the multi-day date range that quietly dropped the weekday (programs), find out why Programs breadcrumb has a link even though there is no index route two images with filenames for alt text, and dead refs in sponsors. there's also about header, hackfest copy, hackathon press links, and programs carousel, breadcrumbs are ugly and uncentered (__root padding/mx-auto space control with staticData?), rendered posts links don't look like regular links (they should have parity) and internal links still do target blank for some reason. maybe add target blank control extension in Showdown. "pending verification" shows even if the sheet reports that verification is complete (when you enter an email into the mailing list that's already there regardless of verification status). use toast.info() to report verified emails, saying how to unsubscribe maybe. animate-out homepage down below hero button is still clickable (not hidden in dom) after scroll. take to top link looks ugly next to rest. footer text doesn't have text-center on mobile. sponsors tabbing is still finicky (tabbing from an element later in the dom using shift+tab brings you to something invisible (other sponsors container?) then StateFarm instead of the last sponsor in the list. Investigate prefers-reduce-motion css animation override (also kind of finnicky, maybe shadcn has docs on accessibility/reduce-motion) and why the css override triggers `Base UI: CSS transitions and CSS animations both detected on Collapsible or Accordion panel. Only one of either animation type should be used.`. make a general "404" component builder to maintain consistency between 404 pages (tag not found, post not found, simple errors like cannot make report or cakeisalie, etc). **IMPORTANT: figure out how to not be dependent on google form prefill and send from form straight to sheet**. prefill is janky due to form changes

## Contributing

Before you commit: for more info on how to (correctly) create posts, the committing guidelines, and more, please see [CONTRIBUTING](CONTRIBUTING.md).

If you need help with something, do not hesitate to DM Ethen on Discord (HackGwinnett officers should have my user). Make sure to send proof you're not an impostor or I probably won't take a look at it.

## Develop and Deploy

We use Vite+, the super-cool next-generation unified tooling system. [Or whatever.](#about-vite)

### Development

`vpr dev`

`vpr` is an alias for `vp run`. It is NOT the same as `vp`... do not run `vp dev` alone

You did it! Now, content-collections is watching for CMS changes AND your dev server is live at port `3000`. Make any changes you want and see them propagate live with Vite.

### Build

`vpr build-static` (or `vpr build-server` for server-enabled*) (or `vpr build-local` locally)

`vpr` is an alias for `vp run`. It is NOT the same as `vp`... do not run `vp build` alone

*YOU **MUST MUST MUST** READ [HOSTING.md](./docs/HOSTING.md) for instructions on how to deploy the build artifact to your hosting provider. The build artifact is located in `dist/` after a successful build.

### Deploy

READ [HOSTING.md](./docs/HOSTING.md) for more. For right now:

The GitHub Action ([deploy.yml](./.github/workflows/deploy.yml)) should handle building and deploying for you on commit to `main`. Boom. Deployed. Use `[skip ci]` in your commit message to skip the build and deploy step for commits that don't need it (like documentation changes).

### About Vite+

This project is using Vite+, a unified toolchain built on top of Vite, Rolldown, Vitest, tsdown, Oxlint, Oxfmt, and Vite Task. Vite+ wraps runtime management, package management, and frontend tooling in a single global CLI called `vp`. Vite+ is distinct from Vite, and it invokes Vite through `vp dev` and `vp build`. Run `vp help` to print a list of commands and `vp <command> --help` for information about a specific command.

---

_Made with_ 🤓 _by [Ethen Tseggai](https://github.com/0xethen)_
